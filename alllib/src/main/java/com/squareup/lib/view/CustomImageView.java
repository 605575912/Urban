...
public class CustomImageView extends ImageView {
    ...
    private void initImagePositionAndSize() {
        ...
        upDateBoundRectF(mBoundRectF);
        float scaleFactor = Math.min(getMeasuredWidth() / mBoundRectF.width(), getMeasuredHeight() / mBoundRectF.height());
        mTotalScaleFactor = 1f;
        mTotalScaleFactor *= scaleFactor;
        mInitialScaleFactor = mTotalScaleFactor;
        ...
        //对图片进行变换，并更新图片的边界矩形
        transform();
        inittop = mBoundRectF.top;
        initleft = mBoundRectF.left;
    }
        ...
        switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mCanTranslate) {
                    ...
                    //平移操作
                    translation(dx, dy, mBoundRectF, mCurrentMatrix, this);
                    //重置上一个单点的坐标
                    mLastSinglePoint.set(event.getX(), event.getY());
                }
                //判断能否缩放操作
     ...
     * 更新矩形边界
     */
    private void upDateBoundRectF(RectF mBoundRectF) {
        if (getDrawable() != null) {
     ...
     * @param dy y方向的位移
     */
    protected void translation(float dx, float dy, RectF mDrawableRectF, Matrix mCurrentMatrix, View view) {
        if (dx == 0 && dy == 0) {
            return;
        }
        float x = 0;
        if (dx <= 0) {
            ...
            float right = mDrawableRectF.right - w;
            if (right > 0) {
                float nright = right + dx;
                if (nright > 0) {
                    x = dx;
                } else {
                    x = -right;
            ...
            float left = mDrawableRectF.left;
            if (left < 0) {
                float nright = left + dx;
                if (nright >= 0) {
                    x = -left;
                } else {
                    x = dx;
        ...
        if (dy <= 0) {
            ...
            float bottom = mDrawableRectF.bottom - h;
            if (bottom > 0) {
                float nbottom = bottom + dy;
                if (nbottom > 0) {
                    y = dy;
                } else {
                    y = -bottom;
    ...
    protected void scale(float scaleFactor) {
        //累乘得到总的的缩放因子
        if (mTotalScaleFactor > mInitialScaleFactor * 3 && scaleFactor >= 1) {
            return;
        }
        mTotalScaleFactor *= scaleFactor;
        mCurrentMatrix.postScale(scaleFactor, scaleFactor, mBoundRectF.centerX(), mBoundRectF.centerY());
...
//        mCurrentMatrix.postRotate(degree, mBoundRectF.centerX(), mBoundRectF.centerY());
    ...
    protected void backScale() {
        ...
        LogUtil.i(mTotalScaleFactor + "===" + mInitialScaleFactor);
        //如果总的缩放比例因子比初始化的缩放因子还小，进行回弹
        if (mTotalScaleFactor < mInitialScaleFactor) {
            //1除以总的缩放因子再乘初始化的缩放因子，求得回弹的缩放因子
            scaleFactor = mInitialScaleFactor / mTotalScaleFactor;
            //更新总的缩放因子，以便下次在此缩放比例的基础上进行缩放
            mTotalScaleFactor = mInitialScaleFactor;

            LogUtil.i("AAA  " + scaleFactor);
            mCurrentMatrix.postScale(scaleFactor, scaleFactor, mBoundRectF.centerX(), mBoundRectF.centerY());
        }
